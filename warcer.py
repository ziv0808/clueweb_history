    ...
    try:
        ...
        try:
            next_str = "HTTP/1.1 200 OK" + "\n" + \
                   ...
                   "Last-Modified: " + parse_timestamp(timstamp) + "\n" + \
                   "Expires: Mon, 20 Dec 1998 01:00:00 GMT" + "\n" + \
                   "Content-Length: " + str(len((html).decode('windows-1252').strip().encode('utf-8')) + 1) + "\n\n" + html.decode('windows-1252').strip().encode('utf-8') + '\n\n'

            record_str += str(len((next_str).decode('windows-1252').strip().encode('utf-8')) + 1) + "\n\n" + next_str
        except Exception as e:
